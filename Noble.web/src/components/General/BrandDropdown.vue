<template>
    <div>
        <multiselect v-model="DisplayValue" :options="options" :disabled="disabled" :multiple="false"
            v-bind:placeholder="$t('BrandDropdown.SelectBrand')" track-by="name" :clear-on-select="false"
            :show-labels="false" label="name" :preselect-first="true">
            <!--<p slot="noResult" class="text-danger"> Oops! No Brand found.</p>-->
            <a slot="noResult" class="btn btn-primary " v-on:click="AddBrand('Add')" v-if="isValid('CanAddBrand')">{{$t('BrandDropdown.CreateProductBrand')}}</a><br />
                ...
                <div class="modal-body">
                    <div class="row ">
                        <div :key="render" class="form-group has-label col-sm-12 "
                            ...
                            <label class="text  font-weight-bolder"> {{ $t('BrandDropdown.Code') }}:<span
                                    class="text-danger"> *</span></label>
                            <input disabled class="form-control"
                                v-bind:class="($i18n.locale == 'en' || isLeftToRight()) ? 'text-left' : 'text-right'"
                                v-model="$v.brand.code.$model" type="text" />
                        ...
                        <div v-if="english == 'true'" class="form-group has-label col-sm-12 "
                            ...
                            <label class="text  font-weight-bolder"> {{ $t('BrandDropdown.BrandName')
                                    | englishLanguage
                            }}: <span class="text-danger"> *</span></label>
                            <input class="form-control" v-bind:class="isLeftToRight() ? 'text-left' : 'arabicLanguage'"
                                v-model="$v.brand.name.$model" type="text" />
                        ...
                        <div v-if="isOtherLang()" class="form-group has-label col-sm-12 "
                            ...
                            <label class="text  font-weight-bolder">{{ $t('BrandDropdown.BrandNameArabic')
                                    | arabicLanguage
                            }}: <span class="text-danger"> *</span></label>
                        ...
                        <div class="form-group has-label col-sm-12 "
                            ...
                            <textarea rows="3" class="form-control"
                                ...
                                v-model="$v.brand.description.$model" type="text" />
...
export default {
    ...
    methods: {
        getData: function () {
            ...
            this.$https.get('/Product/BrandList?isActive=true', { headers: { "Authorization": `Bearer ${token}` } }).then(function (response) {
                ...
                .then(function (response) {
                    ...
                    if (response.data.isSuccess == true) {
                        if (root.type != "Edit") {
                            ...
                            root.$store.state.brandList.push({
                                id: response.data.brand.id,
                                name: (root.$i18n.locale == 'en' || root.isLeftToRight()) ? (response.data.brand.name != '' ? response.data.brand.code + ' ' + response.data.brand.name : response.data.brand.code + ' ' + response.data.brand.nameArabic) : (response.data.brand.nameArabic != '' ? response.data.brand.code + ' ' + response.data.brand.nameArabic : response.data.brand.code + ' ' + response.data.brand.name),
                            ...
                            var data = root.$store.state.brandList.find(function (x) {
